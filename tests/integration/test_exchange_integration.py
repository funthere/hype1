...
class TestHyperliquidAPIIntegration:
    ...
    def mock_exchange_api(self, mock_config):
        ...
        with patch('src.exchange.connector.Exchange') as mock_exchange:
            with patch('src.exchange.connector.Info') as mock_info:
                with patch('src.exchange.connector.Account') as mock_account:
                    # Setup mocks
                    mock_info.return_value = Mock()
                    mock_info.return_value.meta = Mock(return_value={
                        "universe": [
                            {"name": "HYPE"},
                            {"name": "ETH"},
                            {"name": "BTC"}
                    ...
                    mock_exchange_instance.order = Mock(return_value={
                        "status": "ok",
                        "response": {"oid": 12345}
                    })
                    
                    mock_exchange_instance.cancel = Mock(return_value={"status": "ok"})
                    mock_exchange_instance.bulk_cancel = Mock(return_value={"status": "ok"})
                    
                    mock_exchange_instance.update_leverage = Mock(return_value={"status": "ok"})
    ...
    def test_initialization(self, mock_config):
        ...
        with patch('src.exchange.connector.Exchange') as mock_exchange_class:
        ...
        with patch('src.exchange.connector.Exchange') as mock_exchange_class:
            with patch('src.exchange.connector.Info') as mock_info_class:
                with patch('src.exchange.connector.Account'):
                    ...
                    mock_info_class.return_value.meta = Mock(side_effect=Exception("Connection failed"))
    ...
    async def test_get_recent_fills(self, mock_exchange_api):
        ...
        with patch.object(mock_exchange_api.info, 'user_fills') as mock_fills:
            mock_fills.return_value = [
                {"oid": 12345, "coin": "HYPE", "px": 100.0, "sz": 10.0, "time": 1234567890}
            ]
...
class TestExchangeAPIIntegration:
    ...
    def test_exchange_with_api_wallet(self, config_with_api_wallet):
        ...
        with patch('src.exchange.connector.Exchange') as mock_exchange:
            api = HyperliquidAPI(config_with_api_wallet)
            ...
            assert call_kwargs["account_address"] == config_with_api_wallet.ACCOUNT_ADDRESS
    ...
    def test_asset_index_caching(self, mock_config):
        ...
        with patch('src.exchange.connector.Exchange') as mock_exchange:
            with patch('src.exchange.connector.Info') as mock_info:
                ...
                asset_index_1 = api.get_asset_index() if hasattr(api.get_asset_index, '__self__') else None
                ...
                asset_index_2 = api.get_asset_index() if hasattr(api.get_asset_index, '__self__') else None
