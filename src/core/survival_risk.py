...
from ..core.survival import SurvivalConfig
...
class PositionHeatManager:
    ...
    def calculate_position_risk(self, position: Position, entry_price: float) -> float:
        ...
        if position.side == Side.LONG:
            stop_distance = (position.entry_price - position.sl_price) / position.entry_price
        else:
            stop_distance = (position.sl_price - position.entry_price) / position.entry_price
    ...
    def can_add_position(
        ...
        if heat_pct > self.max_heat_per_setup:
            return False, f"Position heat ({heat_pct:.1%}) exceeds limit ({self.max_heat_per_setup:.1%})"

        # Check if we're adding to a losing position
        for pos in existing_positions:
            if (pos.side == new_position.side and
                pos.unrealized_pnl < 0 and
                self.get_setup_key(pos) == self.get_setup_key(new_position)):
...
class VolatilityRiskManager:
    ...
    def get_volatility_multiplier(self) -> float:
        ...
        baseline_atr = np.mean(self.atr_history[-self.atr_window*2:-self.atr_window])
...
class TimeBasedRiskManager:
    ...
    def is_safe_to_trade(self, current_time: Optional[datetime] = None) -> Tuple[bool, str]:
        """
        ...
        if self.avoid_first_last_minutes:
            ...
            if minute < self.avoid_first_last_minutes or minute >= (60 - self.avoid_first_last_minutes):
                # Check if this is a significant time
                if hour in [0, 8, 12, 16, 20]:  # Common trading session starts
                    return False, f"Market hour boundary - avoid ±{self.avoid_first_last_minutes}min"
...
class SurvivalRiskManager:
    ...
    def __init__(self, config: BotConfig):
        ...
        self.drawdown_breaker = MaxDrawdownCircuitBreaker(
    ...
    def can_open_position(
        ...
        if self.drawdown_breaker.is_triggered():
            return False, f"Max drawdown circuit breaker active ({self.drawdown_breaker.get_drawdown_pct():.1%})"
